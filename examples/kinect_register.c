...
void dump_registration(char* regfile) {
  ...
  /*
  printf( "%d\n", dev.reg_info.dx_center );
  printf( "%d\n", dev.reg_info.ax );
  printf( "%d\n", dev.reg_info.bx );
  printf( "%d\n", dev.reg_info.cx );
  printf( "%d\n", dev.reg_info.dx );
  printf( "%f\n", dev.const_shift );
  printf( "%f\n", dev.zero_plane_info.dcmos_emitter_dist );
  printf( "%f\n", dev.zero_plane_info.dcmos_rcmos_dist );
  printf( "%f\n", dev.zero_plane_info.reference_distance );
  printf( "%f\n", dev.zero_plane_info.reference_pixel_size );
  */
  //exit(0);
  ...
  */
  ...
  //printf( "\n\n%d\n\n", sizeof(dev) ); // 176
  //fwrite( &dev, sizeof(dev), 1, fp); // writes the first three fields and the pointers
  ...
  // but for some reason, file produced is 585728 bytes long. Padding? Strange...
...
freenect_registration load_registration(char* regfile) 
{
  ...
  fclose(fp);
  //printf("Loaded regdump\n");

  /*
  printf( "%d\n", reg.reg_info.dx_center );
  printf( "%d\n", reg.reg_info.ax );
  printf( "%d\n", reg.reg_info.bx );
  printf( "%d\n", reg.reg_info.cx );
  printf( "%d\n", reg.reg_info.dx );
  printf( "%f\n", reg.const_shift );
  printf( "%f\n", reg.zero_plane_info.dcmos_emitter_dist );
  printf( "%f\n", reg.zero_plane_info.dcmos_rcmos_dist );
...
void apply_registration(char* regfile, char* PGMfile)
{
  ...
  for (i=0; i<DEPTH_X_RES; i++) {
	for (j=0; j<DEPTH_Y_RES; j++) {
	  ...
	  factor = 2 * ref_pix_size * wz[i*j] / ref_distance;
	  wx[i*j] = (double)(i - DEPTH_X_RES/2) * factor;
	  wy[i*j] = (double)(j - DEPTH_Y_RES/2) * factor;
	} // j
  } // i
