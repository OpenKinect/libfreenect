 ...
 *
 * This file is part of FreeNUI, distributed under the terms of the GNU Lesser General Public License, version 3.
 * See LICENSE in the root of this project for the full text of the license.
 */
#pragma once
...
/// Each entry in `devices` corresponds to a connected libfreenect device.
/// Each Device has a VideoStream `video` and a DepthStream `depth`.
/// Some `devices` might not have motor support (e.g. K4W); in that case `device->motor == nullptr`
///
...
namespace Freenect {
	...
	public:
		...
		Device(unsigned int index) :
			...
			if (ret < 0) {
				...
				motor = std::unique_ptr<Motor>(new Motor(fn_device));
			}
			video = std::unique_ptr<VideoStream>(new VideoStream(fn_device));
			depth = std::unique_ptr<DepthStream>(new DepthStream(fn_device));
		...
		~Device() {
			if (motor) {
				motor->setAngle(0);
				motor->setLED(LED_BLINK_GREEN);
			}
			
			// streams must be stopped before closing device
	...
	namespace {
		...
		std::map< std::string, std::shared_ptr<Device> > stored_devices; // MUST be declared before event_thread
		...
		void run() {
			while (running) {
				...
				for (std::map< std::string, std::shared_ptr<Device> >::iterator iter = stored_devices.begin(); iter != stored_devices.end();) {
					std::shared_ptr<Device>& device = iter->second;
					if (device.unique()) // close unused devices
						iter = stored_devices.erase(iter);
					else { // stop unused streams
						device->depth->verifySlots();
						device->video->verifySlots();
						++iter;
					}
				}
				...
				if (ret < 0) {
					printf("run(): freenect_process_events_timeout returned %d\n", ret);
					//return;
				}
			...
			if (ret < 0)
				printf("run(): freenect_shutdown returned %d\n", ret);
			fn_context = nullptr;
		}
	}
	...
	/// @throw freenect_exception A call to a freenect function returned an error
	std::vector< std::shared_ptr<Device> > devices(unsigned int max_devices = UINT_MAX) {
		...
		for (int index = 0; index < num_devices; ++index, attributes = attributes->next) {
			if (stored_devices.size() >= max_devices)
				break;
			
			printf("camera serial = %s\n", attributes->camera_serial);
			...
			if (! stored_devices.count(attributes->camera_serial))
				stored_devices.insert(std::make_pair(attributes->camera_serial, std::make_shared<Device>(index)));
		}
		...
		for (unsigned int index : k4w_indexes) {
			...
			if (! stored_devices.count(std::to_string(index)))
				stored_devices.insert(std::make_pair(std::to_string(index), std::make_shared<Device>(index)));
		}
		
		std::vector< std::shared_ptr<Device> > devices;
		for (std::pair< std::string, std::shared_ptr<Device> > device : stored_devices) {
