...
int freenect_start_video(freenect_device *dev)
{
	...
	dev->hflip_setting = HFLIP_DISABLE;
	dev->video.running = 1;
	return 0;
}
...
int freenect_set_hflip(freenect_device *dev, freenect_hflip setting)
{
	freenect_context *ctx = dev->parent;
	...
	switch(dev->video_format) {
		...
		case FREENECT_VIDEO_IR_10BIT_PACKED:
			...
			break;
		default:
			FN_ERROR("freenect_set_hflip(): called with invalid video format %d\n", dev->video_format);
			result = -1;
	}

	dev->hflip_setting = setting;

	if ( dev->hflip_setting )
		write_register(dev, hflip_reg, 0x01);
	else
		write_register(dev, hflip_reg, 0x00);
